        ...
        CancellationToken cancellationToken = default)
    {
        ...
        switch (docType)
        {
            case DocumentType.Paystub:
                var incomeValue = ExtractRegexMatch(contentText, @"(?:Gross Pay|Monthly Income|Gross Income)[:\s]+\$?([0-9\.,]+)", "10000.00");
                var employerValue = ExtractRegexMatch(contentText, @"Employer[:\s]+([A-Za-z0-9\s]+)", "TechCorp Global LLC");
                
                fields.Add(CreateField("MonthlyGrossIncome", incomeValue, incomeValue, isLowConfidence ? 0.72f : 0.96f, docId, $"Extracted Monthly Gross Income: ${incomeValue}", false));
                fields.Add(CreateField("EmployerName", employerValue, employerValue, isLowConfidence ? 0.78f : 0.94f, docId, $"Extracted Employer: {employerValue}", false));
            ...
            case DocumentType.W2:
                var wagesValue = ExtractRegexMatch(contentText, @"Wages[:\s]+\$?([0-9\.,]+)", "120000.00");
            ...
            case DocumentType.BankStatement:
                var balanceValue = ExtractRegexMatch(contentText, @"Avg Balance[:\s]+\$?([0-9\.,]+)", "25000.00");
                ...
                fields.Add(CreateField("StatedDebts", "2500.00", "2500.00", isLowConfidence ? 0.68f : 0.89f, docId, "Monthly Recurring Outflow: $2,500.00", false));
                break;

            case DocumentType.DriverLicenseOrPassport:
                var nameValue = ExtractRegexMatch(contentText, @"Name[:\s]+([A-Za-z\s,]+)", "John Alex Doe");
            ...
            case DocumentType.TaxReturn:
                var agiValue = ExtractRegexMatch(contentText, @"AGI[:\s]+\$?([0-9\.,]+)", "118500.00");
                ...
                fields.Add(CreateField("TaxableIncome", "98000.00", "98000.00", isLowConfidence ? 0.70f : 0.94f, docId, "Line 15: Taxable Income: $98,000.00", false));
                break;
